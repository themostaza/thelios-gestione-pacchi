...
import { ChevronDown, ChevronUp, Search, Package, Edit, CheckCircle, XCircle, X, Send, AlertCircle } from 'lucide-react'
...
const PackageManagementTable: FC = () => {
  ...
  const sendNewReminder = (id: number) => {
    setPackages((prev) => prev.map((pkg) => (pkg.id === id ? { ...pkg, emailSent: new Date().toISOString() } : pkg)))
              ...
              {(['arrivato', 'in consegna', 'completato', 'cancellato'] as const).map((status) => (
                <button
                  ...
                  className={`px-3 py-1 rounded-full border text-sm font-medium transition-colors ${
                    selectedStatuses.includes(status) ? statusColors[status] + ' ring-2 ring-offset-1' : 'bg-gray-100 text-gray-700 border-gray-200'
                  }`}
                >
            ...
            <tr>
              ...
              >
                <div className='flex items-center'>
                  Tracking Number
                  {sortConfig.key === 'trackingNumber' && (sortConfig.direction === 'asc' ? <ChevronUp className='h-4 w-4 ml-1' /> : <ChevronDown className='h-4 w-4 ml-1' />)}
                </div>
              </th>
              <th
                scope='col'
              ...
              >
                <div className='flex items-center'>
                  Destinatario
                  {sortConfig.key === 'recipient' && (sortConfig.direction === 'asc' ? <ChevronUp className='h-4 w-4 ml-1' /> : <ChevronDown className='h-4 w-4 ml-1' />)}
                </div>
              </th>
              <th
                scope='col'
              ...
              >
                <div className='flex items-center'>
                  Data Arrivo
                  {sortConfig.key === 'arrivalDate' && (sortConfig.direction === 'asc' ? <ChevronUp className='h-4 w-4 ml-1' /> : <ChevronDown className='h-4 w-4 ml-1' />)}
                </div>
              </th>
              <th
                scope='col'
          ...
          <tbody className='bg-white divide-y divide-gray-200'>
            {filteredPackages.map((pkg) => (
              ...
              >
                ...
                <td className='px-6 py-4 whitespace-nowrap text-sm text-gray-500'>{formatDateTime(pkg.arrivalDate)}</td>
                ...
                <div className='h-full flex flex-col bg-white shadow-xl overflow-y-auto'>
                  ...
                  <div className='flex-shrink-0 px-4 py-5 sm:px-6 bg-gray-50 border-b border-gray-200'>
                    <div className='flex items-center justify-between'>
                      <h2 className='text-lg font-medium text-gray-900'>Dettagli Pacco</h2>
                      <button
                        type='button'
                  ...
                  <div className='flex-1 p-6'>
                    <div className='mb-6'>
                      <div className='flex items-center justify-between mb-2'>
                        <h3 className='text-xl font-semibold text-gray-800'>{selectedPackage.trackingNumber}</h3>
                      ...
                      <div className='grid grid-cols-1 md:grid-cols-2 gap-4 mb-8'>
                        ...
                        <div>
                          ...
                          <select
                            ...
                            onChange={(e) => updatePackageStatus(selectedPackage.id, e.target.value as PackageData['status'])}
              ...
              <div className='bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4'>
                <div className='sm:flex sm:items-start'>
                  ...
                  <div className='mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left'>
                    <h3 className='text-lg leading-6 font-medium text-gray-900'>Conferma completamento</h3>
                    <div className='mt-2'>
                      <p className='text-sm text-gray-500'>Sei sicuro di voler contrassegnare questo pacco come completato? Questa azione indica che il pacco è stato consegnato al destinatario.</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className='inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full'>
              <div className='bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4'>
                <div className='sm:flex sm:items-start'>
                  ...
                  <div className='mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left'>
                    <h3 className='text-lg leading-6 font-medium text-gray-900'>Conferma cancellazione</h3>
                    <div className='mt-2'>
                      <p className='text-sm text-gray-500'>Sei sicuro di voler cancellare questo pacco? Il record rimarrà nel database ma sarà contrassegnato come &quot;cancellato&quot;.</p>
                    </div>
                  </div>
                </div>
              </div>
