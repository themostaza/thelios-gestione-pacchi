...
async function checkAndCreateProfile(supabase, userId, email) {
  try {
        ...
        try {
          if (hasAccessToken) {
            ...
            if (sessionResult.data.session) {
              ...
              await checkAndCreateProfile(supabase, user.id, user.email);
          ...
          const { data: { subscription } } = supabase.auth.onAuthStateChange((event, session) => {
            ...
            if (event === 'SIGNED_IN' && session) {
              // Check/create profile for the user
              checkAndCreateProfile(supabase, session.user.id, session.user.email);
